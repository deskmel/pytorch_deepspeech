...
parser.add_argument('--test-manifest', metavar='DIR',
                    help='path to validation manifest csv', default='data/libri_test_manifest.csv')
parser.add_argument('--batch-size', default=1, type=int, help='Batch size for training')
parser.add_argument('--num-workers', default=4, type=int, help='Number of workers used in dataloading')
parser.add_argument('--verbose', action="store_true", help="print out decoded output and error of each sample" ,default = True)
...
if __name__ == '__main__':
    ...
    for i, (data) in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        if (i==0):
            ...
            mseloss = torch.sum(torch.pow(out_star-out,2)).item()
